...
#   orange for date differences, white for identical files, gray for placeholders.
# - Directories are colored based on differences in their contents.
# - A legend explains the color meanings.
#
# Dependencies:
# - Python 3.x
...
# Bandera global para evitar bucles de sincronización
SYNCING = False
...
def main():
    root = tk.Tk()
    root.title("ZIP Comparator with Synced Trees - v0.3.0")
    ...
    tk.Label(legend_frame, text="ZIP1 only (Light Blue) | ZIP2 only (Light Green) | Content Diff (Yellow) | Date Diff (Orange) | Identical (White) | Placeholder (Gray)", font=('Arial', 10)).pack(side='left')
    ...
    for tree in (tree1, tree2):
        ...
        tree.tag_configure('dir_diff', background='lightcoral')  # Color para directorios con diferencias
