# ZIP Comparator - Version 0.3.1
#
# Description:
# This program provides a graphical interface to compare two ZIP files, displaying their contents
# in two synchronized Treeview widgets side by side. It highlights differences in files and directories
# using colors, with placeholders for missing files to keep trees aligned. Features include:
# - Synchronized scrolling and selection between trees.
# - Expandable directories with synchronized expansion/contraction.
# - Configurable logging levels (DEBUG, INFO, WARNING, ERROR, CRITICAL).
# - Automatic loading of test ZIP files in DEBUG mode.
# - Conditional creation of test ZIP files if they don't exist.
# - Color-coded differences: blue for ZIP1-only, green for ZIP2-only, yellow for content differences,
#   orange for date differences, white for identical files, gray for placeholders, coral for directories with differences.
# - A legend with colored backgrounds explains the color meanings.
# - File selectors are placed in a single line, each above its respective Treeview.
#
# Dependencies:
# - Python 3.x
# - tkinter (included with standard Python installation)
# - zipfile, os, logging, datetime, sys (standard Python modules)
...
def main():
    root = tk.Tk()
    root.title("ZIP Comparator with Synced Trees - v0.3.1")
    ...
    tk.Label(zip1_frame, text="ZIP 1:").pack(side='left', padx=5)
    tk.Entry(zip1_frame, textvariable=zip1_path, width=30).pack(side='left', padx=5)
    tk.Button(zip1_frame, text="Select", command=lambda: select_zip(zip1_path)).pack(side='left', padx=5)

    # Frame para controles de ZIP 2
    zip2_frame = tk.Frame(frame)
    zip2_frame.pack(side='left', padx=10, fill='x', expand=True)
    tk.Label(zip2_frame, text="ZIP 2:").pack(side='left', padx=5)
    tk.Entry(zip2_frame, textvariable=zip2_path, width=30).pack(side='left', padx=5)
    tk.Button(zip2_frame, text="Select", command=lambda: select_zip(zip2_path)).pack(side='left', padx=5)

    # Frame para nivel de log y botón Compare
    control_frame = tk.Frame(root)
    control_frame.pack(pady=5)
    tk.Label(control_frame, text="Log Level:").pack(side='left', padx=5)
    ...
    tk.Button(control_frame, text="Compare", command=lambda: compare(zip1_path.get(), zip2_path.get(), tree1, tree2)).pack(side='left', padx=10)

    # Leyenda de colores
    legend_frame = tk.Frame(root)
    legend_frame.pack(pady=5, fill='x')
    tk.Label(legend_frame, text="Legend: ", font=('Arial', 10, 'bold')).pack(side='left')
    tk.Label(legend_frame, text="ZIP1 only", background='lightblue', font=('Arial', 10)).pack(side='left', padx=2)
    tk.Label(legend_frame, text="|", font=('Arial', 10)).pack(side='left')
    tk.Label(legend_frame, text="ZIP2 only", background='lightgreen', font=('Arial', 10)).pack(side='left', padx=2)
    tk.Label(legend_frame, text="|", font=('Arial', 10)).pack(side='left')
    tk.Label(legend_frame, text="Content Diff", background='yellow', font=('Arial', 10)).pack(side='left', padx=2)
    tk.Label(legend_frame, text="|", font=('Arial', 10)).pack(side='left')
    tk.Label(legend_frame, text="Date Diff", background='orange', font=('Arial', 10)).pack(side='left', padx=2)
    tk.Label(legend_frame, text="|", font=('Arial', 10)).pack(side='left')
    tk.Label(legend_frame, text="Identical", background='white', font=('Arial', 10)).pack(side='left', padx=2)
    tk.Label(legend_frame, text="|", font=('Arial', 10)).pack(side='left')
    tk.Label(legend_frame, text="Placeholder", background='gray95', font=('Arial', 10)).pack(side='left', padx=2)
    tk.Label(legend_frame, text="|", font=('Arial', 10)).pack(side='left')
    tk.Label(legend_frame, text="Dir Diff", background='lightcoral', font=('Arial', 10)).pack(side='left', padx=2)
    ...
    def sync_selection(event):
        ...
        if event.time == LAST_EVENT_TIME:
            logger.debug(f"Skipping sync_selection: Duplicate event time {event.time}")
            return
        LAST_EVENT_TIME = event.time
        logger.debug(f"Processing sync_selection event time: {event.time}")
        ...
        if selected and selected != target_selected:
            logger.debug(f"Updating selection to {selected} in {target_tree}")
            target_tree.selection_set(selected)
            target_tree.see(selected[0])
            logger.debug(f"Synchronized selection to item: {selected[0]}")
        else:
            logger.debug("No selection change needed or no selection")
    ...
    def sync_open(event):
        ...
        global LAST_EVENT_TIME
        if event.time == LAST_EVENT_TIME:
            logger.debug(f"Skipping sync_open: Duplicate event time {event.time}")
            return
        LAST_EVENT_TIME = event.time
        logger.debug(f"Processing sync_open event time: {event.time}")
    ...
    try:
        with zipfile.ZipFile(zip1_file) as z1, zipfile.ZipFile(zip2_file) as z2:
            ...
            for dir_path, tags in dir_tags.items():
                ...
                logger.debug(f"Directory {dir_path} tagged as {tag}")
    ...
    try:
        ...
        if not os.path.exists('test1.zip'):
            with zipfile.ZipFile('test1.zip', 'w', zipfile.ZIP_DEFLATED) as z1:
                z1.writestr('project/docs/readme.txt', 'README for ZIP1')
                z1.writestr('project/docs/install.txt', 'Installation guide v1')
                z1.writestr('project/src/main.py', 'print("Hello from ZIP1")')
                ...
                logger.debug("Created test1.zip with 7 files")

        # ZIP 2
        if not os.path.exists('test2.zip'):
            with zipfile.ZipFile('test2.zip', 'w', zipfile.ZIP_DEFLATED) as z2:
                z2.writestr('project/docs/readme.txt', 'README for ZIP2 (updated)')
                z2.writestr('project/docs/install.txt', 'Installation guide v1')  # Same content
                z2.writestr('project/src/main.py', 'print("Hello from ZIP2")')  # Different content
                z2.writestr('project/src/utils/extra.py', 'def extra(): pass')  # New file
                z2.writestr('project/tests/test_extra.py', 'assert False')  # New file
                z2.writestr('data/images/icon.png', b'\x89PNG\r\nfakeimage')  # Same content
                z2.writestr('data/config.json', '{"version": "1.1"}')  # Different content
                logger.debug("Created test2.zip with 7 files")
